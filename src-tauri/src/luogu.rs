...
fn luogu_problem_content_url(problem_id: &str, kind: &str) -> String {
    match kind {
        "solution" => format!("https://www.luogu.com.cn/problem/solution/{problem_id}?_contentOnly=1"),
        "discussion" => format!("https://www.luogu.com.cn/discuss/lists?forumname={problem_id}&_contentOnly=1"),
...
fn push_luogu_json_text(parts: &mut Vec<String>, label: &str, value: Option<String>) {
    if let Some(text) = value.map(|item| item.trim().to_string()).filter(|item| !item.is_empty()) {
    ...
    match value {
        ...
        JsonValue::Object(map) => {
            for (key, item) in map {
                ...
                if key_lower.contains("html") || key_lower.contains("avatar") || key_lower.contains("image") {
                    continue;
                }
                collect_luogu_strings(item, output, limit);
                if output.len() >= limit {
...
fn extract_luogu_problem_statement(problem_id: &str, json: &JsonValue) -> (String, String) {
    let problem = json.pointer("/data/problem").or_else(|| json.pointer("/currentData/problem")).unwrap_or(json);
    let title = luogu_json_text_at(problem, &["/name", "/contenu/name"]).unwrap_or_else(|| problem_id.to_string());
    let mut parts = vec![format!("# {problem_id} {title}")];
    let contenu = problem.pointer("/contenu").unwrap_or(problem);
    push_luogu_json_text(&mut parts, "Problem Background", luogu_json_text_at(contenu, &["/background"]));
    push_luogu_json_text(&mut parts, "Problem Statement", luogu_json_text_at(contenu, &["/description"]));
    push_luogu_json_text(&mut parts, "Input Format", luogu_json_text_at(contenu, &["/inputFormat", "/input"]));
    push_luogu_json_text(&mut parts, "Output Format", luogu_json_text_at(contenu, &["/outputFormat", "/output"]));
    ...
    if let Some(samples) = contenu.pointer("/samples").and_then(JsonValue::as_array) {
        for (index, sample) in samples.iter().take(3).enumerate() {
            ...
            if !input.trim().is_empty() || !output.trim().is_empty() {
                parts.push(format!("## Sample {}\n\nInput:\n{}\n\nOutput:\n{}", index + 1, input.trim(), output.trim()));
            }
        }
    }
    (title, parts.join("\n\n"))
...
fn extract_luogu_generic_content(problem_id: &str, kind: &str, json: &JsonValue) -> (String, String) {
...
fn extract_luogu_content(problem_id: &str, kind: &str, body: &str) -> Result<(String, String), String> {
    let json = if body.trim_start().starts_with('{') {
        serde_json::from_str::<JsonValue>(body).ok()
    } else {
        extract_luogu_lentille_json(body)
    }
    .ok_or_else(|| "Luogu reader failed: response did not contain readable JSON data".to_string())?;
...
fn send_luogu_content_request(
    ...
    let mut request = client
        ...
        .header(reqwest::header::REFERER, luogu_problem_display_url(problem_id, "problem"))
...
fn read_luogu_problem_content_blocking(
    ...
    let cookie_config = require_luogu_config(&config).ok();
    ...
    if !status.is_success() {
        return Ok(ReadLuoguProblemContentResult {
            ...
            error: Some(format!("Luogu reader failed: server returned HTTP {}", status.as_u16())),
        });
    }
...
#[tauri::command]
pub async fn read_luogu_problem_content(
    input: ReadLuoguProblemContentInput,
) -> Result<ReadLuoguProblemContentResult, String> {
    tauri::async_runtime::spawn_blocking(move || read_luogu_problem_content_blocking(input))
        .await
        .map_err(|e| format!("Luogu reader task failed: {e}"))?
}
