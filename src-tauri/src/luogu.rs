...
use crate::luogu_reader::{
    read_luogu_problem_content_blocking, ReadLuoguProblemContentInput,
...
fn build_ai_prepared_luogu_note(
    ...
    let markdown =
        build_ai_note_markdown(&problem_id, problem_title, submission_id, insight, ai_model);
...
fn prepare_luogu_submission_note_blocking(
    ...
    Ok(prepare_luogu_submission_record_note(
        &client, &uid, &client_id, &notes_dir, &config.ai, &rules, &record,
    ))
}
...
pub fn write_luogu_prepared_note(
    ...
    if let Some(parent) = target_path.parent() {
        fs::create_dir_all(parent)
            .map_err(|e| format!("Luogu write failed: cannot create note parent directory: {e}"))?;
    }
...
pub async fn read_luogu_problem_content(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        read_luogu_problem_content_blocking(input, cookie_config)
    })
    .await
...
"#;
    ...
    fn generated_ai_frontmatter_uses_ai_fields_and_keeps_draft() {
        ...
        let markdown = build_ai_note_markdown(
            "P1234",
            "AI Interval Coverage",
            "987654",
            &insight,
            "deepseek-chat",
        );
