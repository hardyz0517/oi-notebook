...
- `2cfc332 refactor(tasks): add task status predicates`
- `7ab53f5 refactor(notes): centralize rename path rules`
- `refactor(notes): centralize root tree selection rules`
- `refactor(tasks): add paused status predicate`
- `refactor(luogu): centralize import center busy state`
- `refactor(luogu): centralize preparation workspace reset`
- `refactor(tags): centralize normalization scan selectors`
...
- Remaining foundation focus: further `App.tsx` shell decomposition, broader task-controller adoption of `src/lib/taskStatus.ts` beyond Luogu scan/write progress and local index status/load/rebuild state, more note workspace side-effect cleanup where it can stay pure, tag manager config helper consolidation where behavior can be preserved, blog frontend controller helpers if useful, safer Luogu/task-controller consolidation, continued API boundary audits, and documentation/handoff sync. Latest verified slices centralized file-tree selection state, root tree selection state, and rename path rules in `src/lib/noteWorkspace.ts`, tag taxonomy export JSON/file-name rules and entry/alias add-delete update rules in `src/lib/tagTaxonomyUserConfig.ts`, Luogu config form load/save payload rules in `src/lib/luoguConfigForm.ts`, queued/paused/resumed/stopping transition helpers and common status predicates in `src/lib/taskStatus.ts`, Luogu scan state now consumes the shared running/paused/failed predicates, Luogu import center global busy state and preparation workspace reset state are centralized in `src/components/luogu/luoguImportDisplay.ts`, tag normalization scan stats/selection derivation is centralized in `src/components/tag-manager/tagNormalizationScan.ts`, and local index task view consumption of task predicates; `pnpm.cmd test:run`, `pnpm.cmd build`, and the frontend API boundary audit passed afterward.
