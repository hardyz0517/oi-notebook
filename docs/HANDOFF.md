...
- Local blog view model:
  ...
  should render these ready-to-render items, read browser debug flags, and
  perform console side effects instead of owning tag-map, collection
  overview/detail rows, archive list rows, post card/result rows, pagination,
  or diagnostics data-shaping rules directly.
