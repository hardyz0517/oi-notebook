...
- `local-blog/src/blogViewModel.ts`: local-blog tag chip labels, related tag
  ...
  The local-blog App shell should render tag-map, collection overview/detail
  rows, archive list rows, recent update cards, post card/result rows, and
  pagination view data, read browser debug flags, and perform console side
