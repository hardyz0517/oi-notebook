...
- `local-blog/src/blogViewModel.ts`: local-blog tag chip labels, related tag
  ...
  collection overview state/card rows, collection detail header data,
  collection detail entry rows, archive list sections/rows, recent update
  cards, note detail header/navigation context, note navigation items, compact
  post card/result rows, compact pagination items/links, and tag diagnostics
  payloads/debug-enable rules. The local-blog App shell should render tag-map,
  collection overview/detail rows, archive list rows, recent update cards, note
...
only when the extracted rule has a stable owner and focused test value.
