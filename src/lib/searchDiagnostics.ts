...
export const formatSearchPreparationDiagnosticsForDisplay = (raw: string): string[] => {
  ...
  return [
    ...
    diagnostics.targetReadCount ? `目标读取来源数: ${diagnostics.targetReadCount}` : undefined,
    diagnostics.attemptedReadCount ? `尝试读取来源数: ${diagnostics.attemptedReadCount}` : undefined,
    ...
    diagnostics.usableBodyEvidenceCount ? `可用正文证据数: ${diagnostics.usableBodyEvidenceCount}` : undefined,
    diagnostics.usableFreshBodyEvidenceCount ? `可用近期正文证据数: ${diagnostics.usableFreshBodyEvidenceCount}` : undefined,
    diagnostics.currentDate ? `当前日期: ${diagnostics.currentDate}` : undefined,
    diagnostics.freshnessWindowDays ? `时效窗口天数: ${diagnostics.freshnessWindowDays}` : undefined,
    diagnostics.freshnessGateStatus ? `时效性门控状态: ${diagnostics.freshnessGateStatus}` : undefined,
    diagnostics.freshnessFailureReason ? `时效性失败原因: ${diagnostics.freshnessFailureReason}` : undefined,
    diagnostics.freshEvidenceCount ? `近期正文证据数: ${diagnostics.freshEvidenceCount}` : undefined,
    diagnostics.staleEvidenceCount ? `旧资料数: ${diagnostics.staleEvidenceCount}` : undefined,
    diagnostics.unknownDateEvidenceCount ? `日期不明资料数: ${diagnostics.unknownDateEvidenceCount}` : undefined,
    diagnostics.rejectedByFreshnessCount ? `因时效性拒绝数: ${diagnostics.rejectedByFreshnessCount}` : undefined,
    diagnostics.evidenceGateStatus ? `证据门控状态: ${diagnostics.evidenceGateStatus}` : undefined,
