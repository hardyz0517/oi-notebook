...
describe("appShell", () => {
  ...
  it("keeps the Agent Workbench shell mounted so toggling does not reset its local state", () => {
    ...
    expect(appSource).toContain('import { AgentWorkbenchShell } from "@/components/agent-workbench/AgentWorkbenchShell"');
  ...
  it("hides the editor workspace while the mounted Agent Workbench is open", () => {
    const appSource = readFileSync(appSourcePath, "utf8");

    expect(appSource).toContain('isAgentWorkbenchOpen ? "hidden" : "flex"');
    expect(appSource).toContain('isAgentWorkbenchOpen ? "flex" : "hidden"');
  });
