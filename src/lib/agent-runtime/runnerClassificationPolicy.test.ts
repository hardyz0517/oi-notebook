...
describe("P14 runner classification policy", () => {
  ...
  it("classifies supported command classes without executing anything", () => {
    const cases = [
      ...
      ["clang++ main.cpp -std=c++20", "compile"],
  ...
  it("classifies supported test-run classes without spawning runners", () => {
    const cases = [
      ...
      [{ command: "g++ main.cpp -std=c++20", runnerKind: "shell-command", testIntent: "inspect" }, "compile-check"],
      [{ command: "clang++ main.cpp -std=c++20", runnerKind: "shell-command", testIntent: "inspect" }, "compile-check"],
