...
const COMPILE_COMMAND_PATTERN =
  /(?:^|\s)(?:tsc|gcc|javac|rustc)(?:\s|$)|(?:^|\s)(?:g\+\+|clang\+\+)(?:\s|$)|(?:^|\s)cargo\s+check(?:\s|$)/i;
...
export function classifyRunnerCommand(input: ClassifyRunnerCommandInput): RunnerCommandClass {
  ...
  if (input.runnerKind === "compile-run" || isCompileCommand(command)) {
    return "compile";
  }
...
export function classifyRunnerTestRun(input: ClassifyRunnerTestRunInput): RunnerTestRunClass {
  ...
  if (input.runnerKind === "compile-run" || /\bcompile\b/i.test(text) || isCompileCommand(text)) {
...
function isCompileCommand(text: string): boolean {
  return COMPILE_COMMAND_PATTERN.test(text);
}
