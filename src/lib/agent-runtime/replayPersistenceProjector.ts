...
function projectModelLoopEvent(event: DurableAgentEventLogEntry): ModelLoopEvent {
  const type = MODEL_LOOP_EVENT_TYPES.has(event.eventType) ? event.eventType : "observation.added";
...
function projectTimelineEvent(event: DurableAgentEventLogEntry): ReplayPersistenceTimelineEvent {
  return {
    eventId: event.eventId,
    type: MODEL_LOOP_EVENT_TYPES.has(event.eventType) ? event.eventType : "observation.added",
    sequence: event.sequence,
    turnId: event.turnId,
    stepId: event.stepId,
