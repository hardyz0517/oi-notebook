...
describe("P12 replay persistence projector", () => {
  ...
  it("returns structured failures for duplicate sequence, missing sequence, session mismatch, step mismatch, unknown schema, redaction violation and corrupt checkpoint", () => {
    ...
    expect(
      projectDurableReplayLog({
        metadata,
        events: [{ ...validEvents[0], schemaVersion: 999 } as unknown as DurableAgentEventLogEntry],
        checkpoints: [],
      }),
    ).toEqual({
      ok: false,
    ...
    expect(
      projectDurableReplayLog({
        metadata,
        events: validEvents,
        checkpoints: [{ ...createCheckpoint(), schemaVersion: 999 } as unknown as AgentSessionStoreCheckpoint],
      }),
    ).toEqual({
      ok: false,
