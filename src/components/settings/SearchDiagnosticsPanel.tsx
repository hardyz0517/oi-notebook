...
const buildNotexSelfCheckItem = (item: NotexSearchSelfCheckCaseResult): DiagnosticItem => ({
  ...
  summary: `${item.expectedCategory}; intent=${item.actualIntent}; vertical=${item.vertical}; freshness=${item.freshness}; newsRegistry=${item.newsRegistryTriggered}; clustering=${item.newsClusteringTriggered}; clusters=${item.clusterCount}/${item.selectedClusterCount}; localResults=${item.localResultCount}; displayedLocalSources=${item.displayedLocalSourceCount}`,
  detail: item.reason,
  safeDebugInfo: [
