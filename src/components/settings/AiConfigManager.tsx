...
import { ArrowLeft, GripVertical, Loader2, PackagePlus, Pencil, PlugZap, Plus, RefreshCw, Search, Server, Trash2, X } from "lucide-react";
...
export default function AiConfigManager({
  ...
  useEffect(() => {
    ...
    if (!activeProvider) setView("list");
  }, [activeProvider, view]);
  ...
  const managerOverlay = open ? (
    <div className="fixed inset-0 z-[120] bg-black/45" role="presentation">
      <div className="mx-auto my-8 flex h-[calc(100vh-64px)] w-[min(1180px,calc(100vw-64px))] flex-col overflow-hidden rounded-xl border border-border/70 bg-background shadow-2xl">
        {/* ===== Header ===== */}
        <header className="shrink-0 border-b border-border/60 bg-muted/10 px-4 py-2.5">
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <h2 id="ai-config-manager-title" className="font-heading text-base font-semibold tracking-tight text-foreground">
                AI 配置组
              </h2>
            ...
            <div className="flex shrink-0 flex-wrap items-center gap-2">
              ...
              <Button type="button" variant="ghost" size="icon-sm" onClick={() => setOpen(false)} aria-label="关闭 AI 配置组管理中心" title="关闭">
  ...
  const deleteConfirmOverlay = deleteProvider ? (
    <div className="pointer-events-auto fixed inset-0 z-[140] grid place-items-center bg-black/45 px-4" role="presentation">
      <div
    ...
    <>
      <div className="grid gap-3 rounded-md border border-border/70 bg-muted/10 p-4">
        <div className="flex min-w-0 flex-col gap-3 md:flex-row md:items-center md:justify-between">
          ...
          <Button type="button" size="sm" onClick={() => setOpen(true)} disabled={isLoading}>
