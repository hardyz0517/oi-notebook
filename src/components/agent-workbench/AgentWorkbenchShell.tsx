...
export function AgentWorkbenchShell({
  ...
  const submitCurrentTask = (event: FormEvent<HTMLFormElement>): void => {
      ...
      <main className="grid min-h-0 content-start gap-3 overflow-auto">
        <div className="grid gap-2 border border-border/70 bg-background p-4">
          ...
          <div className="grid grid-cols-2 gap-2 text-[11px] text-muted-foreground">
            <div>Runtime: {preview?.runtimeReady ? "ready" : "preview"}</div>
            <div>Workspace: {preview?.workspaceReady ? "ready" : "preview"}</div>
            <div>Research: {preview?.researchBoundaryReady ? "ready" : "preview"}</div>
            <div>Legacy sidebar: {preview?.legacySidebarIsolated ? "isolated" : "unchanged"}</div>
          </div>
        </div>
        <form className="grid gap-2 border border-border/70 bg-background p-3" onSubmit={submitCurrentTask}>
